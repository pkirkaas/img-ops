...
# Converter: Convert string back to int
        ...
        if db_path:
            ...
            self.db_path = Path(platformdirs.user_data_dir(
                app_name, app_author)) / "file_info_cache.db"
        ...
        try:
            ...
            raise sqlite3.Error(
                f"Failed to initialize database at {self.db_path}: {e}") from e
    ...
    def _ensure_db_table(self) -> None:
      ...
      """)
            ...
            self._conn.rollback()
            raise sqlite3.Error(
                f"Failed to create or verify 'file_cache' table: {e}") from e
...
if __name__ == '__main__':
    ...
    print(
        f"Default DB path would be: {Path(platformdirs.user_data_dir('img-ops', 'pkirkaas')) / 'file_info_cache.db'}")
    ...
    with FileInfoCache(db_path=temp_db_path) as cache:
        ...
        print(
            f"PHash for {not_an_image_path.name} (should be None): {phash_txt}")
        assert phash_txt is None

        # Test for a non-existent file
        phash_non_existent = cache.get_phash("non_existent_file.png")
        print(
            f"PHash for non_existent_file.png (should be None): {phash_non_existent}")
        ...
        if dummy_image_path1.exists():
            ...
            print(
                f"PHash for {dummy_image_path1.name} (after modification): {phash1_modified}")
        ...
        for row in cache._cursor.fetchall():
            print(row)
            # Ensure deleted file is gone
            assert Path(row[0]).name != dummy_image_path2.name
