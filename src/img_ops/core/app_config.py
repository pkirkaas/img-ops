  ...
  def validate_paths(cls, v):
    ...
    for path in v:
      if not isinstance(path, str):
        raise ValueError(f"All paths must be strings, got {type(path)}")
...
class AppConfigManager(BaseModel):
  ...
  def get_or_create_configuration(self, name: str) -> AppConfiguration:
    ...
    new_config = AppConfiguration(name=name, description=f"{name} configuration")
...
def get_config_manager(config_file_path: Optional[Union[str, Path]] = None) -> AppConfigManager:
  ...
  if _global_config_manager is None:
    _global_config_manager = AppConfigManager(config_file_path=config_file_path)
