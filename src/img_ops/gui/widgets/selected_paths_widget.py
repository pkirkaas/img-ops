...
from PySide6.QtWidgets import (
    ...
    # Added QStyle for icons, QDialog for list, QProgressDialog
    QStyle, QDialog, QProgressDialog
)
# Added QFileInfo and QObject
from PySide6.QtCore import Qt, Slot, Signal, QPoint, QFileInfo, QObject
...
# Added get_all_image_files_in_paths
from ...core.file_system import check_nested, get_all_image_files_in_paths
...
class SelectedPathsWidget(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.paths_table.setEditTriggers(
            QAbstractItemView.EditTrigger.NoEditTriggers)  # Read-only
        self.paths_table.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu)
        self.paths_table.customContextMenuRequested.connect(
    ...
    def _get_path_icon(self, path_str: str) -> QIcon:
        ...
        if p_info.isFile():
            if self._is_image_file(path_str):
                ...
                # Or SP_MessageBoxQuestion for unexpected file types
                return style.standardIcon(QStyle.StandardPixmap.SP_FileIcon)

        # Default for unknown types
        return style.standardIcon(QStyle.StandardPixmap.SP_QuestionMark)
    ...
    def _is_image_file(self, file_path_str: str) -> bool:
        ...
        for pattern in DEFAULT_IMAGE_EXTENSIONS_PATTERNS:  # From SelectPathDialog
            # e.g., ".jpg" from "*.jpg"
            pattern_ext_lower = pattern[1:].lower()
    ...
    def _add_path_action(self):
        ...
        selected_path = SelectPathDialog.get_path(
            parent=self, title="Add Path to Selection")
        if selected_path:
            # Check for nesting before adding
            # Use app_state.selected_paths directly for the most up-to-date list
            current_paths_list = list(
                self._app_state.selected_paths) if self._app_state else []
    ...
    def _remove_selected_paths_action(self):
        ...
        selected_rows = sorted(list(
            set(index.row() for index in self.paths_table.selectedIndexes())), reverse=True)
        ...
        for row in selected_rows:
            ...
            if item:
                # Assuming item text is the path
                paths_to_remove.append(item.text())
    ...
    def _show_context_menu(self, pos: QPoint):
        ...
        add_action.setIcon(self.style().standardIcon(
            QStyle.StandardPixmap.SP_DialogOpenButton))  # Example Icon
        ...
        if self.paths_table.selectedItems():  # Or check selectedIndexes()
            ...
            remove_action.setIcon(self.style().standardIcon(
                QStyle.StandardPixmap.SP_DialogDiscardButton))  # Example Icon
    ...
    def _list_files_action(self):
        ...
        progress = QProgressDialog(
            "Scanning files...", "Cancel", 0, 0, self)  # Indeterminate
    ...
    def _phash_files_action(self):
        ...
        gather_progress = QProgressDialog(
            "Gathering image files...", "Cancel", 0, 0, self)
        ...
        phash_progress = QProgressDialog(
            "Calculating PHashes...", "Cancel", 0, len(all_image_files), self)
        ...
        for i, file_path_obj in enumerate(all_image_files):
            ...
            try:
                ...
                print(f"Error PHashing {file_path_str}: {e}")
                error_count += 1
...
if __name__ == '__main__':
    ...
    class DummyFileInfoCache:
        ...
        def get_or_compute_phash(self, file_path_str: str) -> Optional[str]:
            if file_path_str in self._cache:
                print(
                    f"PHash for {file_path_str} from cache: {self._cache[file_path_str]}")
            ...
            if Path(file_path_str).exists() and Path(file_path_str).is_file():
                ...
                print(
                    f"Computed dummy PHash for {file_path_str}: {dummy_phash}")
                return dummy_phash
    ...
    class DummyAppState(QObject):
        ...
        def __init__(self):
            ...
            self.file_info_cache: Optional[DummyFileInfoCache] = DummyFileInfoCache(
                self._temp_db_file.name)
            print(
                f"Dummy AppState created with cache: {self._temp_db_file.name}")
        ...
        def cleanup(self):
            ...
            try:
                ...
                print(
                    f"Temporary cache file {self._temp_db_file.name} deleted.")
            except Exception as e:
                print(
                    f"Error deleting temporary cache file {self._temp_db_file.name}: {e}")
