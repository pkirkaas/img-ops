...
class ConfigEditDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.setWindowTitle(
            "Edit Configuration" if self.config.name != "MyConf" else "New Configuration")
        self.setModal(True)
        ...
        self.remove_path_btn.setToolTip(
    ...
    def _connect_signals(self):
        ...
        self.paths_list.itemSelectionChanged.connect(
            self._on_path_selection_changed)
        ...
        for i in range(self.paths_list.count()):
            ...
            if os.path.normpath(existing_path) == normalized_path:
                show_selectable_message_box(
                    self, QMessageBox.Information, "Duplicate Path", f"Path already exists:\n{path}")
                return
    ...
    def _validate_and_accept(self):
        ...
        if not name:
            show_selectable_message_box(
                self, QMessageBox.Warning, "Invalid Name", "Configuration name cannot be empty.")
            self.name_edit.setFocus()
            return

        if name in self.existing_names and name != self.original_name:
            show_selectable_message_box(
                self, QMessageBox.Warning, "Duplicate Name", f"Configuration name '{name}' already exists.")
            self.name_edit.setFocus()
            return
        ...
        try:
            ...
            show_selectable_message_box(
                self, QMessageBox.Critical, "Validation Error", f"Configuration validation failed:\n{str(e)}")
...
class AppConfigWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.config_list.setToolTip(
            "Select a configuration to view or edit. Right-click for more options.")
        self.config_list.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu)  # Enable custom context menu
    ...
    def _connect_signals(self):
        ...
        self.config_list.itemSelectionChanged.connect(
            self._on_config_selection_changed)
        self.config_list.itemDoubleClicked.connect(self._edit_configuration)
        self.config_list.customContextMenuRequested.connect(
            self._show_config_list_context_menu)
    ...
    def _display_config_details(self, config: AppConfiguration):
        ...
        if self.config_manager and self.config_manager.config_file_path:
            # Removed .resolve()
            details.append(
                f"Source File: {self.config_manager.config_file_path}")
        details.append("")
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            try:
                if self.config_manager.add_configuration(new_config):
                    ...
                    show_selectable_message_box(
                        self, QMessageBox.Warning, "Error", f"Configuration '{new_config.name}' already exists.")
            except ConfigError as e:
                show_selectable_message_box(
                    self, QMessageBox.Critical, "Configuration Error", f"Failed to save configuration:\n{str(e)}")
            except Exception as e:
                show_selectable_message_box(
                    self, QMessageBox.Critical, "Unexpected Error", f"An unexpected error occurred:\n{str(e)}")
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                if self.config_manager.remove_configuration(config_name):
                    ...
                    show_selectable_message_box(
                        self, QMessageBox.Warning, "Error", f"Configuration '{config_name}' not found.")
            except ConfigError as e:
                show_selectable_message_box(
                    self, QMessageBox.Critical, "Configuration Error", f"Failed to delete configuration:\n{str(e)}")
            except Exception as e:
                show_selectable_message_box(
                    self, QMessageBox.Critical, "Unexpected Error", f"An unexpected error occurred:\n{str(e)}")
    ...
    def _copy_configuration(self):
        ...
        try:
            ...
            if self.config_manager.add_configuration(new_config):
                ...
                self._select_config_by_name(new_config.name)
                self.configuration_changed.emit(new_config.name)
                show_selectable_message_box(self, QMessageBox.Information, "Configuration Copied",
                                            f"Configuration '{original_name}' copied to '{new_config.name}'.")
            else:
                show_selectable_message_box(self, QMessageBox.Warning, "Copy Error",
                                            f"Could not add copied configuration '{new_name}'. It might already exist.")
        except ConfigError as e:
            show_selectable_message_box(
                self, QMessageBox.Critical, "Copy Error", f"Failed to save copied configuration:\n{str(e)}")
        except Exception as e:
            show_selectable_message_box(
                self, QMessageBox.Critical, "Copy Error", f"An unexpected error occurred while copying:\n{str(e)}")
    ...
    def _on_ok_clicked(self):
        ...
        try:
            ...
            show_selectable_message_box(
                self, QMessageBox.Critical, "Save Error", f"Failed to save configurations:\n{str(e)}")
            return
