...
class SelectPathDialog(QDialog):
  ...
  def __init__(self,
    ...
    self._set_current_path_in_view(start_dir, add_to_history=False) # Initial path, don't add to history yet
  ...
  def _populate_places(self):
    ...
    places = [
        ...
        ("Desktop", QStandardPaths.writableLocation(QStandardPaths.StandardLocation.DesktopLocation), QStyle.StandardPixmap.SP_DesktopIcon),
        ...
        ("This PC", "", QStyle.StandardPixmap.SP_ComputerIcon) # Special case for drives
    ]
    for name, path, icon_enum in places:
        item = QListWidgetItem(name)
        item.setIcon(self.style().standardIcon(icon_enum))
        item.setData(Qt.ItemDataRole.UserRole, path) # Store path in UserRole
    ...
    for drive in QDir.drives(): # QFileInfo objects
        ...
        item = QListWidgetItem(f"Drive {drive.filePath().upper()}") # e.g. "Drive C:/"
        item.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DriveHDIcon))
  ...
  def _on_place_selected(self, item: QListWidgetItem):
    ...
    path_data = item.data(Qt.ItemDataRole.UserRole) # This is the path string, or "" for This PC
  ...
  def _set_current_path_in_view(self, path_str: str, add_to_history: bool = True, is_this_pc_navigation: bool = False):
    ...
    """
    current_model_root_path = self.model.filePath(self.tree_view.rootIndex())
    ...
    target_model_index = self.model.index(path_str) # For path_str="", this gives the "Computer" level
    ...
    if path_str == "": # If we just navigated to "This PC" (empty string path)
        self.tree_view.reset() # Force the view to completely refresh from the model at this new root
    ...
    if not canonical_path_from_model: # This means path_str was "" (This PC)
        self.path_line_edit.setText("This PC")
    else:
        self.path_line_edit.setText(canonical_path_from_model)

    if add_to_history:
        self._add_to_history(canonical_path_from_model) # Store "" for "This PC"
  ...
  def _navigate_up(self):
    ...
    if parent_index.isValid() and parent_path_str != current_path_str:
      ...
      self._set_current_path_in_view(parent_path_str, is_this_pc_navigation=(parent_path_str == ""))
  ...
  def _navigate_back(self):
    ...
    if self._history_index > 0:
        ...
        self._set_current_path_in_view(path_to_go, add_to_history=False) # Don't re-add to history
  ...
  def _update_nav_buttons_state(self):
    ...
    can_go_up = parent_of_root.isValid() and self.model.filePath(parent_of_root) != self.model.filePath(current_root_index)
