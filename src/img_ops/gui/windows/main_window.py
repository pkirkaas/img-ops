...
# Import new widget
from ..widgets.select_configuration_widget import SelectConfigurationWidget
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.app_state.set_selected_paths(
            set(self.app_config.paths))  # Initialize AppState
        ...
        if self.file_info_cache:
            ...
            print(
                "MainWindow: FileInfoCache was not initialized, so not setting it on AppState.")

        # --- Toolbar Setup ---
        self.toolbar = QToolBar("Main Toolbar")
        # Optional: prevent toolbar from being moved
        ...
        v_splitter_main = ResizeContainer(
        ...
        main_content_splitter = ResizeContainer(
            orientation=Qt.Orientation.Horizontal, background_color="lightcoral", parent=v_splitter_main)
        ...
        self.selected_paths_widget_main = SelectedPathsWidget(
            parent=main_content_splitter)
        ...
        main_content_splitter.setWidgetSizes(
            [300, 200])  # Adjust initial sizes as needed

        # Add the main content area
        v_splitter_main.addWidget(main_content_splitter)
        ...
        self.config_display.update_display(
            self.app_config, config_file_path_str)  # Populate the display
    ...
    def _load_config_into_widgets(self):
        ...
        if self.app_config:
            ...
            self.settings_config_widget_main.load_configuration(
                self.app_config)
    ...
    def _connect_signals(self):
        ...
        self.app_state.selected_paths_changed.connect(
            self._on_app_state_paths_changed)

        # Connect the toolbar config selector widget
        self.select_config_widget_toolbar.configuration_selected.connect(
            self._on_toolbar_config_selected)

        # Connect the SettingsConfigWidget's update signal
        self.settings_config_widget_main.configuration_updated.connect(
            self._on_settings_config_updated)
    ...
    def _on_toolbar_config_selected(self, config_name: str):
        ...
        if new_config:
            if self.app_config is None or self.app_config.name != new_config.name:  # Check if it's actually a change
                # MainWindow holds the single source of truth for active config
                self.app_config = new_config
                config_file_path_str = self.config_manager.config_file_path if self.config_manager.config_file_path else None
                self.config_display.update_display(
                    self.app_config, config_file_path_str)
                self.statusBar().showMessage(
                    f"Configuration '{self.app_config.name}' activated.", 3000)

                # Load data into SettingsConfigWidget and paths into SelectedPathsWidget
                self._load_config_into_widgets()
                print(
                    f"MainWindow: Active configuration changed to '{self.app_config.name}' via toolbar.")
        else:
            show_selectable_message_box(
                self,
            ...
            if self.app_config:
                self.select_config_widget_toolbar.set_selected_configuration(
    ...
    def _on_settings_config_updated(self, updated_config: AppConfiguration):
        ...
        self.statusBar().showMessage(
            f"Configuration '{self.app_config.name}' updated.", 3000)
        print(
            f"MainWindow: Configuration '{self.app_config.name}' updated via SettingsConfigWidget.")
    ...
    def _show_config_dialog(self):
        ...
        app_config_widget.configuration_changed.connect(
            lambda changed_config_name: self.select_config_widget_toolbar.refresh_configurations(
                ...
                select_config_name=self.app_config.name
            )
        )
        ...
        if config_dialog.exec() == QDialog.DialogCode.Accepted:
            try:
                ...
                if updated_config_name:
                    self.app_config = self.config_manager.get_configuration(
                        updated_config_name)
                    if self.app_config:
                        config_file_path_str = self.config_manager.config_file_path if self.config_manager.config_file_path else None
                        self.config_display.update_display(
                            self.app_config, config_file_path_str)
                        self.statusBar().showMessage(
                            f"Configuration '{self.app_config.name}' loaded.", 3000)
                        # Update the toolbar selector and load data into widgets
                        self.select_config_widget_toolbar.refresh_configurations(
                            select_config_name=self.app_config.name)
                        self._load_config_into_widgets()
                    else:
                        ...
                        self.select_config_widget_toolbar.refresh_configurations(
                            select_config_name=self.app_config.name)
                        self._load_config_into_widgets()
                        show_selectable_message_box(self, QMessageBox.Icon.Warning, "Configuration Error",
                                                    f"Could not load configuration: {updated_config_name}. Reverted to default.")
                # If no specific config was selected, refresh with current (possibly default)
                else:
                    self.app_config = self.config_manager.get_configuration(
                        self.app_config.name) or self.config_manager.get_default_configuration()
                    config_file_path_str = self.config_manager.config_file_path if self.config_manager.config_file_path else None
                    self.config_display.update_display(
                        self.app_config, config_file_path_str)
                    self.select_config_widget_toolbar.refresh_configurations(
    ...
    def _clear_file_cache(self):
        ...
        if not self.file_info_cache:
            show_selectable_message_box(
                self, QMessageBox.Icon.Warning, "Cache Not Available", "File info cache is not initialized.")
            return
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                show_selectable_message_box(
                    self, QMessageBox.Icon.Information, "Cache Cleared", "All entries have been removed from the cache.")
            except Exception as e:
                show_selectable_message_box(
                    self, QMessageBox.Icon.Critical, "Error Clearing Cache", f"Could not clear the cache: {e}")
    ...
    def _clean_file_cache(self):
        ...
        try:
            ...
            show_selectable_message_box(self, QMessageBox.Icon.Information, "Cache Cleaned",
                                        "Invalid or outdated entries have been removed from the cache.")
        except Exception as e:
            show_selectable_message_box(
                self, QMessageBox.Icon.Critical, "Error Cleaning Cache", f"Could not clean the cache: {e}")
    ...
    def _show_cache_status_dialog(self):
        ...
        """
        if not self.file_info_cache:
            show_selectable_message_box(self, QMessageBox.Icon.Warning, "Cache Not Available",
                                        "File info cache is not initialized. Cannot show status.")
            return

        dialog = CacheStatusDialog(
            cache_instance=self.file_info_cache, parent=self)
        dialog.exec()
